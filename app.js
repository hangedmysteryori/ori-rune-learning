...
function homePage() {
  ...
  return [
    ...
    '<div class="button-row">', button('預約盧恩占卜', SERVICE_LINKS.booking, true), button('了解盧恩課程', SERVICE_LINKS.course), '</div></div></div></section>'
  ].join('');
}
...
function startHerePage() {
  return [
    ...
    '<p>一枚盧恩符文，不只代表一個音，也承載一種力量、一個概念，或一段生命經驗。這也是為什麼盧恩能從古代文字，延伸成現代人用來占卜、自我觀察與理解生命狀態的工具。</p></div></section>',
    ...
    '</div><div class="quote-box map-note">24 枚符文可以被看作一張生命地圖：第一組學會建立力量與連結，第二組學會面對限制與轉化，第三組學會整合關係與命運。</div></div></section>',
    ...
    '<div class="button-row">', button('前往每日一符文', '#/daily', true), '</div></div></section>',
...
function aboutPage() {
  return [
    ...
    '<article class="content-card"><h3>一對一盧恩占卜</h3><p>適合正在面對感情困惑、關係不確定、人生選擇，或想看懂自己內在模式的人。</p><div class="button-row">', button('預約占卜', SERVICE_LINKS.booking, true), '</div></article>',
    '<article class="content-card"><h3>系統化盧恩課程</h3><p>適合想從零開始、建立自己的解讀系統，並將盧恩應用於自我探索與生活指引的人。</p><div class="button-row">', button('了解課程', SERVICE_LINKS.course), '</div></article>',
...
function render() {
  ...
  if (path === '/') {
    ...
    setMeta({
      title: '世界樹觀測者 Ori｜盧恩符文學習',
      description: '世界樹觀測者 Ori 的 Elder Futhark 盧恩符文入門學習網站。',
      ogTitle: '世界樹觀測者 Ori｜盧恩符文學習',
    ...
    setMeta({
      ...
      description: '瀏覽 Elder Futhark 24 枚盧恩符文的符號、名稱、核心關鍵字與基礎含義。',
      ogTitle: '24 枚盧恩符文',
      ogDescription: '認識 Elder Futhark 24 枚盧恩符文的象徵、含義與學習順序。'
    });
  }
  else if (path === '/daily') {
